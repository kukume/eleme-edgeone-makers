...
async function loginBySms(session, smsCode) {
  ...
  const bxUmidtoken = String(fp.bxUmidToken || session.bxUmidtoken || "");
  const postLogin = [];
  if (bxUmidtoken) {
    postLogin.push(`bx-umidtoken=${bxUmidtoken}`);
    postLogin.push(`xqkp=${bxUmidtoken}`);
  }
  if (userId) {
    postLogin.push(`__ebg_uid=${userId}`);
  }
  if (fp.cookies && fp.cookies.isg) {
    postLogin.push(`isg=${fp.cookies.isg}`);
  }
  if (fp.cookies && fp.cookies.tfstk) {
    postLogin.push(`tfstk=${fp.cookies.tfstk}`);
  }
  if (postLogin.length) {
    cookie = mergeCookie(cookie, ...postLogin);
  }
  return {
    cookie,
    userId,
...
var onRequestPost = wrap(smsLogin);
